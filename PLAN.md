...
- **`/admin/reports`** works the reports `/api/social/reports` has been writing since social shipped and nothing read. Hide (a comment soft-deletes; a pour goes private **and its active `/s/` links are revoked**, because `getPublicPourShare` authorises on `revokedAt` alone and would otherwise keep serving exactly what was hidden), suspend, reinstate, dismiss. Every state change and its audit row commit in one transaction. **A profile has no hide**: its only lever is `socialEnabled`, a switch in the account's own settings, so hiding one would last until its owner found the toggle they already have while telling the operator they had acted. A profile is suspended or it is not. Oldest first; `REPORT_SLA_HOURS = 72` and `countBreachedReports` make the target a number rather than a sentence.
- **Suspension is not `socialEnabled`.** Reusing that flag would let a suspended account lift its own suspension from its own settings. `suspendedAt`/`suspendedReason` are separate, only an operator writes them, and `setSocialEnabled` refuses to turn social back **on** while one stands. Suspending also clears `phoneDiscoverable`, for the same reason `makeEverythingPrivate` does: every other flag is one the account sets again deliberately, but a stored number would become findable again on reinstatement without ever being asked. Reinstating leaves the flag off: coming back is the user's choice to make again. **Suspended accounts stand in their own list on `/admin/reports`** — suspending resolves the report, and a resolved report leaves the queue with the only reinstate control on it, so an appeal arriving later through `/support` would have had nowhere to land. The reason is readable by the account it is about, on `/sharing`; a reason stored where its subject cannot see it does not tell them what to appeal. An operator cannot suspend themselves — that is a one-keystroke outage with no recovery short of a deploy.
