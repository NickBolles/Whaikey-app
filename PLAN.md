...
- Retention: native auth codes deleted on redemption and swept; rate-limit rows swept after 48 h (`sweepExpiredCounters`, added in WP-18 — the Privacy Policy claimed this and nothing did it); phone probes swept per `social.ts`; chat history kept until the user deletes it; **telemetry — AI token meter readings and the three share-link events — swept after 90 days (`sweepTelemetry`, WP-19)**. The 90 days is the window the numbers are read over (a week for the guardrails, a month for the funnel) plus room to compare quarters; the sweep was written before the policy sentence, which is WP-18's lesson applied in the right order.
