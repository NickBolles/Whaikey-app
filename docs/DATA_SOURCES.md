...
**Cross-source dedupe at ingest time** (src/lib/ingest/index.ts + `matchKey` in normalize.ts): candidates first match by exact slug (bottle id, name, alias), then by a conservative variant match key that collapses age-statement spellings ("12 YO"/"12 yrs"/"12 Years Old"/"12 år"), a leading "The", "Single Malt", and trailing style fillers — but never below two distinguishing tokens (so a brand-only alias can't swallow "Redemption Bourbon" vs "Redemption Rye"), and never on a key two bottles share. Every variant match writes the spelling into `bottle_aliases` (idempotent), so the catalog self-improves: the next sync, search, and scan all resolve that variant directly. Matched bottles get **fill-if-null** attribute merging (abv, ageYears, avgPrice, region — existing values never overwritten); disagreements on abv (>0.5), ageYears, or region are reported as conflicts for human review, while price differences across states are expected and not reported.
...
- **Vinmonopolet** (src/lib/ingest/vinmonopolet.ts) — official Norwegian monopoly developer API (free key → `VINMONOPOLET_API_KEY`). The sanctioned products API has been **slimmed to names only** (no category/ABV/price), and its name filter accepts single words only — the adapter unions whisky-signal terms (~1k products) and infers conservative name-cue categories (no-cue rows land as "world" for enrichment to refine). Value: European name coverage + "sold at a state monopoly" evidence for §2.4b verification.
...
Bulk-ingested bottles (Iowa/COLA) arrive without flavor profiles, so recommendations skip them. **Implemented:** `pnpm ingest enrich` (src/lib/ingest/enrich.ts) fills 8-wedge profiles in tiers: (1) bottles with ≥2 tagged user tasting notes roll up directly from the notes — free, and community truth always wins; (2) the rest go to a Claude model (`WHAIKEY_ENRICH_MODEL` to override) in batches with catalog description + user-note snippets as context. `OPENROUTER_API_KEY` is preferred; `ANTHROPIC_API_KEY` remains a direct-provider fallback. Direct Anthropic enrichment uses its hosted web search tool by default; OpenRouter's compatible Messages endpoint runs this flow without that Anthropic-specific tool, retaining catalog context and model knowledge. Profiles only — no AI-generated descriptions or facts (product guardrails). The `catalog sync` GitHub workflow (.github/workflows/catalog-sync.yml) runs iowa → the §2.2b secondary sources (oregon, utah, bc, systembolaget, whiskyedition, vinmonopolet — each best-effort) → cola → enrich against the production DB weekly and on demand; Vinmonopolet needs the `VINMONOPOLET_API_KEY` repo secret and is skipped without it.

### 2.4b Bulk "actually sold" verification — durable queue + worker execution
Iowa/COLA-imported bottles are label approvals or wholesale SKUs, not proof a consumer product ever shipped — `bottles.status = "imported"` means unverified.

**Retail triage (implemented, July 2026):** appearing in a real retail listing IS sold-evidence, so ingest now short-circuits most of the queue: candidates from retail sources (Iowa, Oregon, Utah, BC, Systembolaget, Vinmonopolet, WHISKY:EDITION reviews) carry `retailEvidence` and are born `verified` — or promote an existing imported bottle on match — with an idempotent `bottle_verifications` row citing the listing. TTB COLA candidates never carry evidence (a label approval is not a sale), so the model-backed queue below now serves only the COLA-derived long tail. **Implemented:** `pnpm verify-sold` (scripts/verify-sold.ts) runs a durable, subscription-only queue (src/lib/ingest/verification-queue.ts): `--dry-run` previews what a snapshot would enqueue (no writes, no Claude calls); `--apply` snapshots imported bottles into per-bottle work rows, then a bounded in-process worker pool (src/lib/ingest/verification-worker.ts, ≤4 concurrent workers) claims leased batches and asks an authenticated Claude Code CLI subscription — never an API key — to confirm each bottle against a real (non-TTB) product page. Every claimed id gets exactly one durable outcome: source-backed evidence promotes the bottle (`src/lib/ingest/verify-sold.ts`'s `persistSoldVerification`) and marks the work row `verified`; an explicit "not sold" answer is durably `not_evidenced` (the bottle stays `imported` and is never re-selected, unlike a naive repeated `status=imported` scan); malformed/missing/duplicate answers and CLI failures bound-retry with backoff before landing on `failed_terminal`. A tiny structured-output smoke probe runs before every `--apply` to catch a logged-out CLI before any lease is claimed. `--report`/`--resume <run-id>` recompute the same seven-field checkpoint (scanned/leased/verified/not_evidenced/retryable/rejected/errors) from durable state, so a crashed invocation is safe to inspect or continue.
