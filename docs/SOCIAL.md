...
6. **Aggregates are built only from what people published, and only when they are really aggregates.** Every community number — the bottle page's average rating, the flavor consensus, anything that follows — reads pours whose owner marked them public *and* whose owner still has social enabled, and is suppressed until at least **three distinct people** are behind it. Both halves matter: without the visibility filter a "community average" quietly carries pours marked Only me (this was live until review SEC-M2), and without the floor a two-person average on a rarely-rated bottle is one person's rating, readable by anyone with the bottle id and no account. The floor counts *people*, not pours, because three pours from one enthusiast are one opinion.
7. **Export includes your social data** — your notes, your comments, your graph. Same free-forever rule.
8. **Age gate at signup covers social.** **⚠️ Not built (verified 2026-09): no age gate exists in the codebase.** Building one is a launch blocker owned by PLAN.md §9.1. When it ships, one gate at signup is the gate; no separate re-check ceremony at first social action — social surfaces expose nothing age-worse than the journal itself.
