...
8. **Age gate at signup covers social.** Built (WP-17): a date of birth and a market, asked once, enforced in the root layout and in `requireUser` so it covers every page and every write. One gate is the gate — there is no re-check at first social action, because social surfaces expose nothing age-worse than the journal itself.
