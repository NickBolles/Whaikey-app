...
- **The unfinished banner** is the only unusual element: while the legal entity, jurisdiction, contact address **or effective date** are unset, the page opens with a visible notice **naming only the ones actually missing**. It is not a soft "coming soon" — the point is that a policy which *looks* finished and names nobody is worse than one that admits it, and a notice that claims the company is unnamed on a page that names it is how a reader learns to stop believing the notice. The date counts because a document with no date from which it binds is not launch-ready, and because the header says "Not yet in effect — see the note below" without one: that sentence has to have a note to point at. It disappears when the environment supplies all four.
