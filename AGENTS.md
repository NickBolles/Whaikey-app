...
- **Notifications**: one pipeline for web push and native push. `sendNotification()` in `src/lib/notifications/deliver.ts` is the only way to send; it resolves category default → account → device, honours each device's own quiet window *and zone*, and logs every attempt (suppressions included) to `notification_deliveries`. Transports sit behind `getPushSender()` / `setPushSenderForTests()` in `sender.ts`. Category ids in `categories.ts` are persisted in jsonb — add, never rename. Missing VAPID/FCM keys must degrade to a visible "Server not configured", never to silent no-ops.
