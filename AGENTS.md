...
- **The stance bans an axis, not fun.** Badges, maps, completions, passports, rivalry with friends, shareable cards — all explicitly encouraged (docs/SOCIAL.md §3.2, spec'd in docs/FEATURES.md §11). What they may count is **distinct things met** — regions, countries, distilleries, cask types, descriptors — never how much or how often. Breadth saturates where volume doesn't, so a 15 ml sample of something new counts the same as a bottle and a repeat pour counts for nothing. The test before building any game mechanic: **can someone win this by drinking less of more?** If yes, build it; do not stop to ask whether gamification is allowed.
